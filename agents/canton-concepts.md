...
- Splice node names have a 30-character max. The validator backend name appends `-validator_backend`
  (18 chars), so validator names are capped at 12 characters by `ValidatorConfigSchema` to avoid
  crashing Splice.
- A config `users[].id` may match the validator's auto-generated default user or wallet-admin user;
  `generateValidatorRealm` emits that Keycloak user only once. Duplicate `users[].id` within one
  validator is rejected by schema validation.
